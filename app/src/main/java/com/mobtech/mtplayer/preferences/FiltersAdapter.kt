...
import com.mobtech.mtplayer.databinding.FilterItemBinding
...
class FiltersAdapter(val activity: Activity) :
    ...
    private val mAvailableItems =
        AppPreferences.getPrefsInstance().filters?.sorted()?.toMutableList()

    private val mDisabledColor = Theming.resolveWidgetsColorNormal(activity)

    private val mDefaultTextColor =
        Theming.resolveColorAttr(activity, android.R.attr.textColorPrimary)
    ...
    inner class CheckableItemsHolder(private val binding: FilterItemBinding) :
