...
import com.mobtech.mtplayer.utils.MusicUtils
...
class MusicViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private fun buildLibrary(resources: Resources) {
        ...
        AppPreferences.getPrefsInstance().filters?.let { filter ->
            deviceMusicFiltered = deviceMusicFiltered?.filter { music ->
                !filter.contains(music.artist) and !filter.contains(music.album) and !filter.contains(
                    music.relativePath
                )
            }?.toMutableList()
        }
        ...
        deviceMusicFiltered?.let { dsf ->
            dsf.filterNot { AppPreferences.getPrefsInstance().filters?.contains(it.artist)!! }
        ...
        val AppPreferences = AppPreferences.getPrefsInstance()
        deviceMusicFiltered?.let { deviceMusic ->
            AppPreferences.queue?.run {
                AppPreferences.queue = this.filter { deviceMusic.contains(it.copy(startFrom = 0)) }
            }
            AppPreferences.favorites?.run {
                AppPreferences.favorites =
                    this.filter { deviceMusic.contains(it.copy(startFrom = 0)) }
            }
        }
    }
}
