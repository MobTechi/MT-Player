...
import com.mobtech.mtplayer.extensions.toFormattedYear
...
object MusicUtils {
    ...
    @JvmStatic
    fun updateMediaPlayerHolderLists(
        mediaPlayerHolder: MediaPlayerHolder,
        ...
        fun selectNewSong(filter: Set<String>): Music? {
            if (musicListContains(currentSong, filter)) {
                AppPreferences.getPrefsInstance().latestPlayedSong = randomMusic
                return randomMusic
            }
            return null
        }
        ...
        AppPreferences.getPrefsInstance().filters?.let { ft ->
            AppPreferences.getPrefsInstance().favorites?.toMutableList()?.let { fav ->
                val songs = fav.filter { favFt ->
                    musicListContains(favFt, ft)
                }
                fav.removeAll(songs.toSet())
                AppPreferences.getPrefsInstance().favorites = fav
                if (fav.isEmpty()) {
    ...
    @JvmStatic
    fun musicListContains(song: Music?, filter: Set<String>) =
        filter.contains(song?.artist) || filter.contains(song?.album) || filter.contains(song?.relativePath)
}
