...
import com.mobtech.mtplayer.databinding.AlbumItemBinding
import com.mobtech.mtplayer.databinding.FragmentDetailsBinding
import com.mobtech.mtplayer.databinding.GenericItemBinding
import com.mobtech.mtplayer.extensions.*
import com.mobtech.mtplayer.models.Album
...
class DetailsFragment : Fragment(), SearchView.OnQueryTextListener {
    ...
    private var mLaunchedBy = AppConstants.ARTIST_VIEW
    private val sLaunchedByArtistView get() = mLaunchedBy == AppConstants.ARTIST_VIEW
    private val sLaunchedByFolderView get() = mLaunchedBy == AppConstants.FOLDER_VIEW
    private val sLaunchedByAlbumView get() = mLaunchedBy == AppConstants.ALBUM_VIEW
    ...
    private val sShowDisplayName get() = AppPreferences.getPrefsInstance().songsVisualization == AppConstants.FN
    ...
    override fun onCreateView(
        ...
        _detailsFragmentBinding = if (sLaunchedByAlbumView) {
            val view = LayoutInflater.from(requireContext())
                .inflate(R.layout.fragment_details_alt_land, container, false)
    ...
    private fun setupViews(view: View) {
        ...
        if (sLaunchedByArtistView) {
            ...
            _detailsFragmentBinding?.sortButton?.run {
                ...
                setOnClickListener {
                    ...
                    setSongsDataSource(
                        ...
                        updateSongs = !sAlbumSwapped && sCanUpdateSongs,
                        updateAdapter = true
                    )
                }
            }
            ...
            _detailsFragmentBinding?.run {
                ...
                if (sLaunchedByFolderView) {
                    ...
                    albumViewArt.background.alpha = Theming.getAlbumCoverAlpha()
                    ...
                    firstSong?.albumId?.waitForCover(requireContext()) { bmp, error ->
                        albumViewArt.loadWithError(bmp, error, R.mipmap.ic_launcher)
                    }
                }
                            ...
                            albumViewCoverContainer.afterMeasured {
                                animate()?.let { anim ->
                                    ...
                                    if (hasFocus) {
                                        ...
                                        anim.withEndAction {
                                            albumViewCoverContainer.handleViewVisibility(
                                                show = false
                                            )
                                        }
                                    } else {
        ...
        view.afterMeasured {
            if (AppPreferences.getPrefsInstance().isAnimations) {
                _detailsFragmentBinding?.root?.run {
    ...
    @SuppressLint("NotifyDataSetChanged")
    private fun setSongsDataSource(
        ...
        _detailsFragmentBinding?.detailsToolbar?.let { tb ->
            ...
            with(tb.menu) {
                when {
                    sLaunchedByArtistView -> {
                        ...
                        findItem(R.id.action_shuffle_sa).isEnabled =
                            mSelectedAlbum?.music?.size!! >= 2
                    }
            ...
            tb.setOnMenuItemClickListener {
                ...
                when (it.itemId) {
                    ...
                    R.id.default_sorting -> applySortingToMusic(AppConstants.DEFAULT_SORTING)
                    R.id.ascending_sorting -> applySortingToMusic(AppConstants.ASCENDING_SORTING)
                    R.id.descending_sorting -> applySortingToMusic(AppConstants.DESCENDING_SORTING)
                    R.id.track_sorting -> applySortingToMusic(AppConstants.TRACK_SORTING)
                    R.id.track_sorting_inv -> applySortingToMusic(AppConstants.TRACK_SORTING_INVERTED)
                    R.id.date_added_sorting -> applySortingToMusic(AppConstants.DATE_ADDED_SORTING)
                    R.id.date_added_sorting_inv -> applySortingToMusic(AppConstants.DATE_ADDED_SORTING_INV)
                    R.id.artist_sorting -> applySortingToMusic(AppConstants.ARTIST_SORTING)
                    R.id.artist_sorting_inv -> applySortingToMusic(AppConstants.ARTIST_SORTING_INV)
    ...
    private inner class AlbumsAdapter : RecyclerView.Adapter<AlbumsAdapter.AlbumsHolder>() {
        ...
        inner class AlbumsHolder(private val binding: AlbumItemBinding) :
            ...
            fun bindItems(itemAlbum: Album?) {
                ...
                with(binding) {
                    ...
                    image.background.alpha = Theming.getAlbumCoverAlpha()

                    itemAlbum?.music?.first()?.albumId?.waitForCover(requireContext()) { bmp, error ->
                        image.loadWithError(bmp, error, R.mipmap.ic_launcher)
                    }
    ...
    private inner class SongsAdapter : RecyclerView.Adapter<SongsAdapter.SongsHolder>() {
        ...
        inner class SongsHolder(private val binding: GenericItemBinding) :
            RecyclerView.ViewHolder(binding.root) {
            ...
            fun bindItems(itemSong: Music?) {
                ...
                with(binding) {
                    ...
                    subtitle.text = if (sLaunchedByFolderView) {
                        getString(
                            R.string.duration_artist_date_added, duration,
                    ...
                    root.setOnLongClickListener {
                        Popups.showPopupForSongs(
                            requireActivity(),
                            _detailsFragmentBinding?.songsRv?.findViewHolderForAdapterPosition(
