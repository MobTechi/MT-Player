...
class MusicNotificationManager(private val playerService: PlayerService) {
    ...
    private fun getPendingIntent(playerAction: String): PendingIntent {
        ...
        return PendingIntent.getService(
            playerService,
            AppConstants.NOTIFICATION_INTENT_REQUEST_CODE,
            intent,
            flags
        )
    }

    private val notificationActions: NotificationAction get() = AppPreferences.getPrefsInstance().notificationActions

    fun createNotification(onCreated: (Notification) -> Unit) {

        mNotificationBuilder =
            NotificationCompat.Builder(playerService, AppConstants.NOTIFICATION_CHANNEL_ID)

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            createNotificationChannel(AppConstants.NOTIFICATION_CHANNEL_ID)
        }
        ...
        val contentIntent = PendingIntent.getActivity(
            playerService, AppConstants.NOTIFICATION_INTENT_REQUEST_CODE,
            openPlayerIntent, flags
        )
        ...
        mNotificationBuilder
            ...
            .addAction(getNotificationAction(AppConstants.PREV_ACTION))
            .addAction(getNotificationAction(AppConstants.PLAY_PAUSE_ACTION))
            .addAction(getNotificationAction(AppConstants.NEXT_ACTION))
    ...
    fun updateNotification() {
        if (::mNotificationBuilder.isInitialized) {
            ...
            with(mNotificationManagerCompat) {
                notify(AppConstants.NOTIFICATION_ID, mNotificationBuilder.build())
            }
        }
    }
    ...
    fun cancelNotification() {
        with(mNotificationManagerCompat) {
            cancel(AppConstants.NOTIFICATION_ID)
        }
    }
    ...
    fun updatePlayPauseAction() {
        if (::mNotificationBuilder.isInitialized) {
            mNotificationActions[2] =
                getNotificationAction(AppConstants.PLAY_PAUSE_ACTION)
        }
    }
    ...
    fun updateRepeatIcon() {
        if (::mNotificationBuilder.isInitialized) {
            mNotificationActions[0] =
                getNotificationAction(AppConstants.REPEAT_ACTION)
            updateNotification()
        }
    }
    ...
    fun updateFavoriteIcon() {
        if (::mNotificationBuilder.isInitialized) {
            mNotificationActions[0] =
                getNotificationAction(AppConstants.FAVORITE_ACTION)
            updateNotification()
        }
    }
    ...
    fun createNotificationForError() {
        ...
        val notificationBuilder =
            NotificationCompat.Builder(playerService, AppConstants.NOTIFICATION_CHANNEL_ERROR_ID)

        createNotificationChannel(AppConstants.NOTIFICATION_CHANNEL_ERROR_ID)
        ...
        with(NotificationManagerCompat.from(playerService)) {
            notify(AppConstants.NOTIFICATION_ERROR_ID, notificationBuilder.build())
        }
    }
