...
class PlayerService : Service() {
    ...
    fun configureMediaSession() {
        ...
        val mediaButtonReceiverComponentName =
            ComponentName(applicationContext, MediaBtnReceiver::class.java)
            ...
            intent?.action?.let { act ->
                ...
                with(mMediaPlayerHolder) {
                    when (act) {
                        ...
                        AppConstants.FAVORITE_POSITION_ACTION -> Lists.addToFavorites(
                            this@PlayerService,
                            currentSong,
                            canRemove = false,
                            playerPosition,
                        ...
                        AppConstants.REWIND_ACTION -> fastSeek(isForward = false)
                        AppConstants.PREV_ACTION -> instantReset()
                        AppConstants.PLAY_PAUSE_ACTION -> resumeOrPause()
                        AppConstants.NEXT_ACTION -> skip(isNext = true)
                        AppConstants.FAST_FORWARD_ACTION -> fastSeek(isForward = true)
                        AppConstants.REPEAT_ACTION -> {
                            repeat(updatePlaybackStatus = true)
                            mediaPlayerInterface.onUpdateRepeatStatus()
                        }
                        AppConstants.CLOSE_ACTION -> if (isRunning && isMediaPlayer) {
