...
import com.mobtech.mtplayer.*
import com.mobtech.mtplayer.databinding.MainActivityBinding
import com.mobtech.mtplayer.databinding.PlayerControlsPanelBinding
import com.mobtech.mtplayer.dialogs.Dialogs
import com.mobtech.mtplayer.dialogs.NowPlaying
import com.mobtech.mtplayer.dialogs.RecyclerSheet
import com.mobtech.mtplayer.equalizer.EqualizerActivity
import com.mobtech.mtplayer.equalizer.EqualizerFragment
import com.mobtech.mtplayer.extensions.*
import com.mobtech.mtplayer.fragments.AllMusicFragment
import com.mobtech.mtplayer.fragments.DetailsFragment
import com.mobtech.mtplayer.fragments.ErrorFragment
import com.mobtech.mtplayer.fragments.MusicContainersFragment
import com.mobtech.mtplayer.models.Music
import com.mobtech.mtplayer.player.MediaPlayerHolder
import com.mobtech.mtplayer.player.MediaPlayerInterface
import com.mobtech.mtplayer.player.PlayerService
import com.mobtech.mtplayer.preferences.SettingsFragment
import com.mobtech.mtplayer.utils.*
...
class MainActivity : BaseActivity(), UIControlInterface, MediaControlInterface {
    ...
    // Booleans
    private val sDetailsFragmentExpanded get() = supportFragmentManager.isFragment(AppConstants.DETAILS_FRAGMENT_TAG)
    private var sAllowCommit = true
    private val sLaunchedByTile get() = intent != null && intent.getStringExtra(AppConstants.LAUNCHED_BY_TILE) != null
    ...
    private fun handleBackPressed() {
        when {
            sDetailsFragmentExpanded -> closeDetailsFragment(isAnimation = mAppPreferences.isAnimations)
    ...
    override fun onSaveInstanceState(outState: Bundle) {
        ...
        sAllowCommit = false
        outState.putInt(AppConstants.RESTORE_FRAGMENT, mMainActivityBinding.viewPager2.currentItem)
    }
            ...
            when (requestCode) {
                AppConstants.PERMISSION_REQUEST_READ_EXTERNAL_STORAGE -> {
                    ...
                    if ((grantResults.isNotEmpty() && grantResults.first() != PackageManager.PERMISSION_GRANTED)) {
                        ...
                        notifyError(AppConstants.TAG_NO_PERMISSION)
                        return
                    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (sLaunchedByTile) {
            mMainActivityBinding.appIcon.handleViewVisibility(false)
            return
        }
        ...
        savedInstanceState?.run {
            mTabToRestore = getInt(AppConstants.RESTORE_FRAGMENT, -1)
        }

        if (intent.hasExtra(AppConstants.RESTORE_FRAGMENT) && mTabToRestore == -1) {
            mTabToRestore = intent.getIntExtra(AppConstants.RESTORE_FRAGMENT, -1)
        }
    }
    ...
    private fun notifyError(errorType: String) {
        ...
        mMainActivityBinding.mainView.animate().apply {
            withStartAction {
                ...
                mMainActivityBinding.appIcon.handleViewVisibility(show = false)
            ...
            withEndAction {
                if (sAllowCommit) {
                    supportFragmentManager.addFragment(
                        ...
                        AppConstants.ERROR_FRAGMENT_TAG
                    )
                }
            }
        }
    ...
    private fun finishSetup(music: MutableList<Music>?) {
        ...
        notifyError(AppConstants.TAG_NO_MUSIC)
    }
    ...
    private fun initViewPager() {
        ...
        mMainActivityBinding.viewPager2.offscreenPageLimit =
            AppConstants.DEFAULT_ACTIVE_FRAGMENTS.size.minus(1)
        mMainActivityBinding.viewPager2.adapter = pagerAdapter
        mMainActivityBinding.viewPager2.registerOnPageChangeCallback(object :
...
//            }
    ...
    private fun initTabLayout() {
        ...
        with(mPlayerControlsPanelBinding.tabLayout) {
            ...
            TabLayoutMediator(this, mMainActivityBinding.viewPager2) { tab, position ->
                tab.setIcon(Theming.getTabIcon(AppConstants.DEFAULT_ACTIVE_FRAGMENTS[position]))
            }.attach()
    ...
    private fun initFragmentAt(position: Int): Fragment {
        when (AppConstants.DEFAULT_ACTIVE_FRAGMENTS[position]) {
            AppConstants.SONGS_TAB -> mAllMusicFragment = AllMusicFragment.newInstance()
            AppConstants.FOLDERS_TAB -> mFoldersFragment = MusicContainersFragment.newInstance(
    ...
    private fun getFragmentForIndex(index: Int) =
        when (AppConstants.DEFAULT_ACTIVE_FRAGMENTS[index]) {
            AppConstants.SONGS_TAB -> mAllMusicFragment ?: initFragmentAt(index)
            AppConstants.FOLDERS_TAB -> mFoldersFragment ?: initFragmentAt(index)
        ...
        if (!sDetailsFragmentExpanded) {
            mDetailsFragment =
                DetailsFragment.newInstance(
                    ...
                    selectedSongId,
                    canUpdateSongs = mMediaPlayerHolder.currentSong?.artist == selectedArtistOrFolder && mMediaPlayerHolder.launchedBy == AppConstants.ARTIST_VIEW
                )
            sCloseDetailsFragment = true
            if (sAllowCommit) {
                supportFragmentManager.addFragment(
                    mDetailsFragment,
                    AppConstants.DETAILS_FRAGMENT_TAG
                )
            }
        }
    }
    ...
    private fun initMediaButtons() {
        ...
        with(mPlayerControlsPanelBinding.favoritesButton) {
            safeClickListener {
                if (!mAppPreferences.favorites.isNullOrEmpty() && mFavoritesDialog == null) {
            ...
            setOnLongClickListener {
                if (!mAppPreferences.favorites.isNullOrEmpty()) {
    ...
    override fun onPlaybackSpeedToggled() {
        ...
        val isPlaybackPersisted =
            mAppPreferences.playbackSpeedMode != AppConstants.PLAYBACK_SPEED_ONE_ONLY
        var playbackSpeed = 1.0F
        if (isPlaybackPersisted) {
            playbackSpeed = mAppPreferences.latestPlaybackSpeed
        }
    ...
    override fun onFavoritesUpdated(clear: Boolean) {
        ...
        val favorites = mAppPreferences.favorites?.toMutableList()

        if (clear) {
            favorites?.clear()
            mAppPreferences.favorites = null
            mFavoritesDialog?.dismissAllowingStateLoss()
        }
    ...
    private fun restorePlayerStatus() {
        ...
        with(mMediaPlayerHolder) {
            ...
            isSongFromPrefs = mAppPreferences.latestPlayedSong != null

            var isQueueRestored = mAppPreferences.isQueue
            if (!mAppPreferences.queue.isNullOrEmpty()) {
                queueSongs = mAppPreferences.queue?.toMutableList()!!
                setQueueEnabled(enabled = true, canSkip = false)
            }

            val song = if (isSongFromPrefs) {
                val songIsAvailable = songIsAvailable(mAppPreferences.latestPlayedSong)
                if (!songIsAvailable.first && isQueueRestored != null) {
                    mAppPreferences.isQueue = null
                ...
                if (!songs.isNullOrEmpty()) {
                    ...
                    mPlayerControlsPanelBinding.songProgress.setProgressCompat(
                        if (isCurrentSongFM) {
                            ...
                            restoredSong.startFrom
                        },
                        true
                    )
                    return
                }
                notifyError(AppConstants.TAG_SD_NOT_READY)
            }
        }
    }
    ...
    private fun saveRandomSongToPrefs(): Music? {
        ...
        mAppPreferences.latestPlayedSong = randomMusic
        return randomMusic
    }
    ...
    private fun getSongSource(): String? {
        ...
        return when (mMediaPlayerHolder.launchedBy) {
            AppConstants.FOLDER_VIEW -> selectedSong?.relativePath
            AppConstants.ARTIST_VIEW -> selectedSong?.artist
    ...
    override fun onOpenSleepTimerDialog() {
        if (mSleepTimerDialog == null) {
            mSleepTimerDialog = RecyclerSheet.newInstance(
                if (mMediaPlayerHolder.isSleepTimer) {
    ...
    override fun onAddToQueue(song: Music?) {
        with(mMediaPlayerHolder) {
            ...
            song?.let { songToQueue ->
                ...
                if (!isPlaying || state == AppConstants.PAUSED) {
    ...
    override fun onAddAlbumToQueue(
        ...
        if (checkIsPlayer(showError = true) && !songs?.isEmpty()!!) {
            ...
            with(mMediaPlayerHolder) {
                ...
                // don't add duplicates
                addSongsToNextQueuePosition(
                    if (restoreQueueSong != songs.first()) {
    ...
    override fun onAddToFilter(stringsToFilter: List<String>?) {
        ...
        MusicUtils.updateMediaPlayerHolderLists(
            mMediaPlayerHolder,
            this,
            mMusicViewModel.getRandomMusic()
        )?.let { song ->
            ...
            with(mMediaPlayerHolder) {
                isPlay = isPlaying
                updateCurrentSong(song, songs, AppConstants.ARTIST_VIEW)
    ...
    override fun onFiltersCleared() {
        AppPreferences.getPrefsInstance().filters = null
        Theming.applyChanges(this, mMainActivityBinding.viewPager2.currentItem)
    }
            ...
            if (Intent.ACTION_VIEW == it.action) {
                it.data?.let { returnUri ->
                    ...
                    try {
                        ...
                        cursor.moveToFirst()
                        mMediaPlayerHolder.currentSongFM =
                            mMusicViewModel.getSongFromIntent(cursor.getString(displayNameIndex))
                        onSongSelected(
                            mMediaPlayerHolder.currentSongFM,
                            null,
                            AppConstants.ARTIST_VIEW
                        )

                    } catch (e: Exception) {
                        e.printStackTrace()
                        notifyError(AppConstants.TAG_NO_MUSIC_INTENT)
                    }
                }
            }
        }
    ...
    private val mMediaPlayerInterface = object : MediaPlayerInterface {
        ...
        override fun onStateChanged() {
            ...
            if (mMediaPlayerHolder.state != AppConstants.RESUMED && mMediaPlayerHolder.state != AppConstants.PAUSED) {
                updatePlayingInfo(restore = false)
        ...
        override fun onBackupSong() {
            if (checkIsPlayer(showError = false) && !mMediaPlayerHolder.isPlaying) {
                mAppPreferences.latestPlayedSong = mMediaPlayerHolder.currentSong?.toSavedMusic(
    ...
    private inner class ScreenSlidePagerAdapter(fa: FragmentActivity) : FragmentStateAdapter(fa) {
        override fun getItemCount() = AppConstants.DEFAULT_ACTIVE_FRAGMENTS.size
